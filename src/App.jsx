    ...
    <div style={{position:'relative', overflow:'hidden', borderRadius:12, marginBottom:8}}>
      ...
      <div
        ...
        style={{
          ...
          background:'#EEF2FF',
          border:`1px solid ${s.done?'#3de87a33':'#C7D2FE'}`,
  ...
  return (
    <div style={{position:'relative', overflow:'hidden', borderRadius:12, marginBottom:8}}>
      ...
      <div
        ...
        style={{transform:`translateX(${offset}px)`, transition: offset===0 ? 'transform 0.25s ease' : 'none', background:'#EEF2FF', borderRadius:12, border:'1px solid #C7D2FE', overflow:'hidden', position:'relative', zIndex:1, userSelect:'none'}}
      >
        <div style={{background:'#E0E7FF', padding:'8px 14px', display:'flex', alignItems:'center', gap:8, borderBottom:'1px solid #E0E7FF'}}>
...
function DayTimeline({ sessions, clients, onClientClick }) {
  ...
  return (
    <div style={{background:'#FFFFFF', border:'1px solid #C7D2FE', borderRadius:14, padding:16, marginTop:12}}>
        ...
        <div style={{flex:1, position:'relative', height: HOURS.length * SLOT_H}}>
          ...
          {HOURS.map((h, i) => (
            <div key={h} style={{
              ...
              background: h === now.getHours() ? 'rgba(200,255,71,.25)' : '#C7D2FE',
            }}/>
          ))}
            ...
            return (
              <div key={s.id} onClick={()=>onClientClick&&c&&onClientClick(c.id)} style={{
                ...
                <div style={{display:'flex', alignItems:'center', gap:5}}>
                  ...
                  <span style={{fontSize:11,fontWeight:600,color:'#111827',overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis',flex:1}}>
                    {isplit ? c?.name?.split(' ')[0] : c?.name}
                  </span>
                  {s.done && <span style={{color:'#3de87a',fontSize:9,flexShrink:0}}>✓</span>}
                </div>
                {s.dur >= 45 && (
                  <div style={{fontSize:9,color:'#9CA3AF',marginTop:2,marginLeft:25}}>
                    {s.time} · {s.dur}хв
                  </div>
                )}
              </div>
...
function DurationPicker({ value, onChange }) {
  ...
  return (
    <div style={{position:'relative', height:containerH, overflow:'hidden', borderRadius:12, background:'#EEF2FF', border:'1px solid #C7D2FE'}}>
      {/* Selection highlight */}
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,height:ITEM_H,
        background:'linear-gradient(to bottom, #EEF2FF, transparent)',pointerEvents:'none',zIndex:3}}/>
      <div style={{position:'absolute',bottom:0,left:0,right:0,height:ITEM_H,
        background:'linear-gradient(to top, #EEF2FF, transparent)',pointerEvents:'none',zIndex:3}}/>
      {/* Scroll container */}
      <div
        ref={scrollRef}
        onScroll={onScroll}
...
function ClipTab({ c, clients, setClients, sessions, pricePlans, setFinance }) {
  ...
  return (
    <div style={{background:'#EEF2FF',borderRadius:12,padding:14}}>
      ...
      {editDateIdx !== null && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300}} onClick={()=>setEditDateIdx(null)}>
          <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:16,padding:24,width:280}} onClick={e=>e.stopPropagation()}>
            <div style={{fontFamily:'Bebas Neue',fontSize:20,marginBottom:16,color:'#111827'}}>Змінити дату #{editDateIdx+1}</div>
            <input type="date" value={editDateVal} onChange={e=>setEditDateVal(e.target.value)}
              style={{width:'100%',padding:'10px 12px',borderRadius:10,border:'1px solid #C7D2FE',background:'#F8FAFF',color:'#111827',fontSize:14,marginBottom:16,boxSizing:'border-box'}}/>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setEditDateIdx(null)}
                style={{flex:1,padding:'9px',borderRadius:10,border:'1px solid #C7D2FE',background:'transparent',color:'#9CA3AF',fontSize:13,cursor:'pointer'}}>
                Скасувати
              </button>
      ...
      {!showAllPlans ? (
        <div style={{marginBottom:14}}>
          ...
          {activePlan ? (
            <>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:'#FFFFFF',border:'1.5px solid rgba(196,237,0,.3)',borderRadius:10,padding:'11px 14px',marginBottom:8}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:'#111827'}}>{activePlan.name}</div>
              ...
              <button onClick={()=>setShowAllPlans(true)}
                style={{width:'100%',padding:'8px',borderRadius:9,border:'1px solid #C7D2FE',background:'transparent',color:'#9CA3AF',fontSize:12,cursor:'pointer'}}>
                🔄 Змінити тариф
              </button>
            </>
          ) : (
            <button onClick={()=>setShowAllPlans(true)}
              style={{width:'100%',padding:'10px',borderRadius:9,border:'1px dashed #C7D2FE',background:'transparent',color:'#9CA3AF',fontSize:13,cursor:'pointer'}}>
              + Обрати тариф
            </button>
          )}
        </div>
            ...
            {pricePlans.map(p=>(
              <div key={p.id} onClick={()=>selectPlan(p)}
                style={{display:'flex',alignItems:'center',gap:10,padding:'11px 14px',borderRadius:11,cursor:'pointer',
                  border:`1.5px solid ${c.active_plan_id===p.id?'#C4ED00':'#C7D2FE'}`,
                  background:c.active_plan_id===p.id?'rgba(196,237,0,.15)':'#FFFFFF'}}>
                <div style={{flex:1}}>
      ...
      {activePlan && !showAllPlans && (
        <>
          ...
          <div style={{height:5,background:'#E0E7FF',borderRadius:3,marginBottom:12,overflow:'hidden'}}>
              ...
              return (
                <div key={i} onClick={()=>isDone && openEditDate(i)}
                  ...
                  <div style={{width:34,height:34,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,
                    background:isDone?'#C4ED00':i===c.clip_used?'rgba(71,212,255,.1)':'#E0E7FF',
            ...
            <div style={{borderTop:'1px solid #E0E7FF',paddingTop:12,marginBottom:12}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:5}}>
                {[...clipDates].reverse().map((date,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:10,background:'#FFFFFF',borderRadius:9,padding:'8px 12px'}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontSize:12,fontWeight:600,color:'#111827'}}>{date.slice(8,10)}/{date.slice(5,7)}/{date.slice(0,4)}</div>
                    </div>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={useClip} style={{flex:1,padding:'9px',borderRadius:10,border:'1px solid #C7D2FE',background:'#F8FAFF',color:'#111827',fontFamily:'DM Sans',fontSize:12,fontWeight:600,cursor:'pointer'}}>Відмітити</button>
...
function PickClientModal({ sessions, clients, onPick, onClose }) {
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:300}} onClick={onClose}>
      <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:'20px 20px 0 0',width:'100%',maxWidth:480,padding:'20px 20px 36px'}} onClick={e=>e.stopPropagation()}>
        <div style={{width:40,height:4,background:'#C7D2FE',borderRadius:2,margin:'0 auto 18px'}}/>
        <div style={{fontFamily:'Bebas Neue',fontSize:22,marginBottom:16}}>Кого редагувати?</div>
        {sessions.map(s => {
          const c = clients.find(x => x.id === s.client_id)
          return (
            <div key={s.id} onClick={() => onPick(s)}
              style={{display:'flex',alignItems:'center',gap:12,padding:'13px 14px',background:'#EEF2FF',borderRadius:12,marginBottom:8,cursor:'pointer',border:'1px solid #C7D2FE'}}>
        ...
        <button onClick={onClose} style={{width:'100%',marginTop:4,padding:11,borderRadius:12,border:'1px solid #C7D2FE',background:'transparent',color:'#9CA3AF',fontFamily:'DM Sans',fontSize:13,cursor:'pointer'}}>Скасувати</button>
      </div>
    </div>
  )
}
...
function EditSessionModal({ session, clients, onClose, onSave, onDelete }) {
  ...
  const inp = {width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,outline:'none',boxSizing:'border-box'}
      ...
      <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:'20px 20px 0 0',width:'100%',maxWidth:480,padding:'20px 20px 36px'}} onClick={e=>e.stopPropagation()}>
          ...
          {clients.map(c => (
            <div
              ...
              style={{
                ...
                border:`2px solid ${clientId===c.id ? c.color : '#C7D2FE'}`,
                background: clientId===c.id ? c.color+'18' : '#EEF2FF',
        ...
        {!confirmDelete ? (
          ...
          <div style={{background:'rgba(255,79,79,.08)',border:'1px solid rgba(255,79,79,.25)',borderRadius:12,padding:14}}>
            <div style={{color:'#111827',fontSize:13,textAlign:'center',marginBottom:12}}>Видалити цю сесію?</div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={() => setConfirmDelete(false)} style={{flex:1,padding:'9px',borderRadius:10,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',fontFamily:'DM Sans',fontSize:13,cursor:'pointer'}}>Скасувати</button>
...
function ScheduleTab({ clients, sessions, setSessions, onClientClick }) {
  ...
  const card = {background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16,marginBottom:14}

  return (
    <div>
      {/* Header */}
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
        <button onClick={prevPeriod} style={{padding:'7px 12px',borderRadius:8,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',cursor:'pointer',fontSize:14}}>‹</button>
        <button onClick={nextPeriod} style={{padding:'7px 12px',borderRadius:8,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',cursor:'pointer',fontSize:14}}>›</button>
        ...
        <button onClick={goToday} style={{padding:'6px 12px',borderRadius:8,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#9CA3AF',cursor:'pointer',fontSize:12,fontWeight:600}}>Сьогодні</button>
        <div style={{display:'flex',borderRadius:8,overflow:'hidden',border:'1px solid #C7D2FE'}}>
          <button onClick={()=>setViewMode('week')} style={{width:80,padding:'6px 0',border:'none',background:viewMode==='week'?'#C4ED00':'#EEF2FF',color:viewMode==='week'?'#111':'#9CA3AF',cursor:'pointer',fontSize:12,fontWeight:600}}>Тиждень</button>
          <button onClick={()=>setViewMode('month')} style={{width:80,padding:'6px 0',border:'none',background:viewMode==='month'?'#C4ED00':'#EEF2FF',color:viewMode==='month'?'#111':'#9CA3AF',cursor:'pointer',fontSize:12,fontWeight:600}}>Місяць</button>
        </div>
      </div>
        ...
        <div style={card}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:12}}>
            {weekDates.map((d,i)=>{
              ...
              return (
                <div key={i} onClick={()=>setSelDs(ds)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3,padding:'8px 2px',borderRadius:10,cursor:'pointer',border:`1px solid ${isSel?'#C4ED00':isToday?'#C4ED0044':'#C7D2FE'}`,background:isSel?'#C4ED00':isToday?'rgba(200,255,71,.08)':'#EEF2FF',color:isSel?'#111':'#F3F4F6',transition:'all .18s'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            ...
            <small style={{color:'#9CA3AF',fontSize:12}}>{daySessions.length} сесій</small>
          </div>

          {/* Sessions */}
          {daySessions.length===0 && <div style={{color:'#9CA3AF',textAlign:'center',padding:'16px 0',fontSize:14}}>Немає сесій</div>}
          ...
          <div onClick={()=>{setFClient(clients[0]?.id||'');setShowModal(true)}} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderRadius:12,border:'1px dashed #C7D2FE',cursor:'pointer',color:'#9CA3AF',fontSize:13,marginTop:4}}>＋ Додати сесію</div>
      ...
      {viewMode==='month' && (
        <div style={card}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2,marginBottom:4}}>
            {DAYS_SHORT.map(d=><div key={d} style={{textAlign:'center',fontSize:11,color:'#9CA3AF',fontWeight:600,padding:'4px 0'}}>{d}</div>)}
          </div>
          ...
          <div style={{borderTop:'1px solid #E0E7FF',paddingTop:12}}>
            ...
            {daySessions.length===0 && <div style={{color:'#9CA3AF',textAlign:'center',padding:'12px 0',fontSize:13}}>Немає сесій</div>}
            ...
            <div onClick={()=>{setFClient(clients[0]?.id||'');setShowModal(true)}} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderRadius:10,border:'1px dashed #C7D2FE',cursor:'pointer',color:'#9CA3AF',fontSize:12,marginTop:4}}>＋ Додати сесію</div>
      ...
      {showModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:20}}>
          <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:20,width:'100%',maxWidth:480,padding:24}}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:12}}>
              <button onClick={()=>setSplitMode(false)} style={{flex:1,padding:'8px',borderRadius:10,border:`1px solid ${!splitMode?'#C4ED00':'#C7D2FE'}`,background:!splitMode?'rgba(200,255,71,.1)':'none',color:!splitMode?'#C4ED00':'#9CA3AF',fontFamily:'DM Sans',fontSize:12,fontWeight:600,cursor:'pointer'}}>👤 Один клієнт</button>
              <button onClick={()=>setSplitMode(true)} style={{flex:1,padding:'8px',borderRadius:10,border:`1px solid ${splitMode?'#C4ED00':'#C7D2FE'}`,background:splitMode?'rgba(200,255,71,.1)':'none',color:splitMode?'#C4ED00':'#9CA3AF',fontFamily:'DM Sans',fontSize:12,fontWeight:600,cursor:'pointer'}}>👥 Спліт (двоє)</button>
            </div>
            ...
            <select value={fClient} onChange={e=>setFClient(e.target.value)} style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,marginBottom:12,outline:'none'}}>
            ...
            {splitMode && (
              <>
                ...
                <select value={fClient2} onChange={e=>setFClient2(e.target.value)} style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,marginBottom:12,outline:'none'}}>
                  <option value="">— Оберіть —</option>
                  {clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
              <div>
                ...
                <input type="time" value={fTime} onChange={e=>setFTime(e.target.value)} style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,outline:'none'}}/>
              </div>
              <div>
                <label style={{fontSize:11,color:'#9CA3AF',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:6}}>Тип</label>
                <input value={fType} onChange={e=>setFType(e.target.value)} placeholder="Силові…" style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,outline:'none'}}/>
              </div>
            </div>
            ...
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>{setShowModal(false);setSplitMode(false)}} style={{flex:1,padding:'10px',borderRadius:10,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',fontFamily:'DM Sans',fontSize:13,fontWeight:600,cursor:'pointer'}}>Скасувати</button>
...
function ClientsTab({ clients, setClients, sessions, setSessions, records, setRecords, pricePlans, setFinance, openClientId, clearOpenClientId }) {
  ...
  const inp = {width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 14px',color:'#111827',fontFamily:'DM Sans',fontSize:14,outline:'none'}
  const lbl = {fontSize:11,color:'#9CA3AF',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:6}
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:12}}>
        {filtered.length===0 && <div style={{color:'#9CA3AF',textAlign:'center',padding:'40px 0',gridColumn:'1/-1'}}>Клієнтів ще немає</div>}
        {filtered.map(c=>{
          ...
          return (
            <div key={c.id} style={{background:'#FFFFFF',border:`1px solid ${isOpen?'#C4ED00':'#C7D2FE'}`,borderRadius:14,overflow:'hidden',alignSelf:'start'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
                  <div style={{display:'flex',gap:6,alignItems:'center'}}>
                    <button onClick={e=>{e.stopPropagation();setEc({name:c.name,goal:c.goal,w:c.weight,h:c.height,started:c.started_at||'',phone:c.phone||'',telegram:c.telegram||''});setEditClient(c)}}
                      style={{background:'none',border:`1px solid #C7D2FE`,borderRadius:7,color:'#9CA3AF',fontSize:12,padding:'3px 7px',cursor:'pointer'}}>✏️</button>
                  ...
                  <span style={{color:'#9CA3AF',fontSize:14,transform:isOpen?'rotate(180deg)':'none',transition:'transform .2s'}}>▾</span>
                </div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:8,padding:'0 14px 14px'}}>
                <div style={{flex:1,height:4,background:'#E0E7FF',borderRadius:2}}>
              ...
              {isOpen && (
                <div style={{borderTop:'1px solid #E0E7FF',padding:14}}>
                  <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
                    {DTABS.map(t=>(
                      <button key={t.id} onClick={()=>setTab(c.id,t.id)} style={{padding:'6px 12px',borderRadius:20,fontSize:12,fontWeight:600,cursor:'pointer',border:`1px solid ${activeTab===t.id?'#C4ED00':'#C7D2FE'}`,background:activeTab===t.id?'#C4ED00':'none',color:activeTab===t.id?'#111':'#9CA3AF'}}>{t.label}</button>
                    ))}
                  </div>
                  {activeTab==='profile' && (
                    <div>
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:12}}>
                        {[['Вага',`${c.weight} кг`],['Зріст',`${c.height} см`],['З нами',c.started_at ? c.started_at.slice(0,10).split('-').reverse().join('.') : '—']].map(([l,v])=>(
                          <div key={l} style={{background:'#EEF2FF',borderRadius:10,padding:10,textAlign:'center'}}>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
                        <div style={{background:'#EEF2FF',borderRadius:10,padding:10}}>
                          ...
                          <textarea defaultValue={(c.strengths||[]).join('\n')} onBlur={e=>updateStrengths(c.id,e.target.value)} placeholder="По одному на рядок" style={{width:'100%',background:'#E0E7FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'8px',color:'#111827',fontFamily:'DM Sans',fontSize:12,resize:'none',outline:'none',minHeight:70,lineHeight:1.5}}/>
                        </div>
                        <div style={{background:'#EEF2FF',borderRadius:10,padding:10}}>
                          <div style={{fontSize:11,fontWeight:600,color:'#ff4f4f',marginBottom:6,textTransform:'uppercase',letterSpacing:.5}}>⚠️ Слабкі</div>
                          <textarea defaultValue={(c.weaknesses||[]).join('\n')} onBlur={e=>updateWeaknesses(c.id,e.target.value)} placeholder="По одному на рядок" style={{width:'100%',background:'#E0E7FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'8px',color:'#111827',fontFamily:'DM Sans',fontSize:12,resize:'none',outline:'none',minHeight:70,lineHeight:1.5}}/>
                        </div>
                      </div>
                      <div style={{fontSize:11,color:'#9CA3AF',textTransform:'uppercase',letterSpacing:.5,marginBottom:6}}>📝 Нотатка</div>
                      <textarea defaultValue={c.note} onBlur={e=>updateNote(c.id,e.target.value)} style={{width:'100%',background:'#F8FAFF',border:'1px solid #C7D2FE',borderRadius:10,padding:'10px 12px',color:'#111827',fontFamily:'DM Sans',fontSize:13,resize:'none',outline:'none',minHeight:80,lineHeight:1.5}}/>
                      {(c.phone||c.telegram) && (
                        <div style={{display:'flex',gap:8,marginTop:12}}>
                          {c.phone && (
                            <button onClick={()=>{navigator.clipboard.writeText(c.phone);alert('Номер скопійовано!')}}
                              style={{flex:1,padding:'10px 8px',borderRadius:10,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',fontFamily:'DM Sans',fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                              📞 {c.phone}
                            </button>
                          )}
                          {c.telegram && (
                  ...
                  {activeTab==='metrics' && (() => {
                    ...
                    return (
                      <div>
                        {cMetrics.length === 0 && (
                          <div style={{color:'#9CA3AF',textAlign:'center',padding:'20px 0',fontSize:13}}>Показників ще немає</div>
                        )}
                        {cMetrics.map(metric => {
                          ...
                          return (
                            <div key={metric.id}
                              ...
                              style={{background:'#EEF2FF',borderRadius:12,padding:'12px 14px',marginBottom:8,cursor:'pointer',border:'1px solid #C7D2FE',display:'flex',alignItems:'center',gap:12}}>
                              <div style={{flex:1}}>
                        ...
                        <button onClick={() => setShowAddMetric(c.id)}
                          style={{width:'100%',marginTop:4,padding:'10px',borderRadius:10,border:'1px dashed #C7D2FE',background:'none',color:'#9CA3AF',fontFamily:'DM Sans',fontSize:13,cursor:'pointer'}}>
                          ＋ Додати показник
                        </button>
                      </div>
                    )
                    ...
                    <div>
                      ...
                      {SCHEDULE_DAYS.map((day,i)=>{
                        ...
                        return (
                          <div key={i} style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                            <button onClick={()=>toggleScheduleDay(c,i)} style={{width:44,padding:'8px 0',borderRadius:10,border:`1px solid ${active?'#C4ED00':'#C7D2FE'}`,background:active?'#C4ED00':'#EEF2FF',color:active?'#111':'#9CA3AF',fontFamily:'DM Sans',fontSize:13,fontWeight:600,cursor:'pointer',flexShrink:0}}>{day}</button>
                            {active && <input type="time" defaultValue={timeVal} onBlur={e=>updateScheduleTime(c,i,e.target.value)} style={{background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'7px 10px',color:'#111827',fontFamily:'DM Sans',fontSize:13,outline:'none',width:110}}/>}
                      ...
                      <div style={{marginTop:16}}>
                        ...
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10}}>
                          <div>
                            <label style={{fontSize:11,color:'#9CA3AF',display:'block',marginBottom:4}}>З дати</label>
                            <input type="date" id={`fill-from-${c.id}`} defaultValue={todayStr()} style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'8px 10px',color:'#111827',fontFamily:'DM Sans',fontSize:13,outline:'none'}}/>
                          </div>
                          <div>
                            <label style={{fontSize:11,color:'#9CA3AF',display:'block',marginBottom:4}}>До дати</label>
                            <input type="date" id={`fill-to-${c.id}`} defaultValue={(() => { const d=new Date(); d.setMonth(d.getMonth()+1); return dateToStr(d) })()} style={{width:'100%',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:8,padding:'8px 10px',color:'#111827',fontFamily:'DM Sans',fontSize:13,outline:'none'}}/>
                          </div>
                        </div>
                  ...
                  {activeTab==='records' && (
                    <div>
                      {cRecords.length===0 && <div style={{color:'#9CA3AF',textAlign:'center',padding:'16px 0',fontSize:14}}>Рекордів ще немає</div>}
                      {cRecords.map(r=>(
                        <div key={r.id} style={{display:'flex',alignItems:'center',gap:10,background:'#EEF2FF',borderRadius:10,padding:'10px 12px',marginBottom:6}}>
                          <div style={{flex:1}}>
                          ...
                          <button onClick={()=>deleteRecord(r.id)} style={{background:'none',border:'none',color:'#9CA3AF',cursor:'pointer',fontSize:16,padding:'0 4px'}}>✕</button>
                        </div>
                      ))}
                      <button onClick={()=>setShowAddRecord(c.id)} style={{width:'100%',marginTop:8,padding:'10px',borderRadius:10,border:'1px dashed #C7D2FE',background:'none',color:'#9CA3AF',fontFamily:'DM Sans',fontSize:13,cursor:'pointer'}}>＋ Додати рекорд</button>
                    </div>
                  )}
                  {activeTab==='clip' && (
                  ...
                  {activeTab==='history' && (
                    <div>
                      {cSessions.filter(s=>s.done).sort((a,b)=>b.date.localeCompare(a.date)).map(s=>(
                        <div key={s.id} style={{display:'flex',alignItems:'center',gap:10,background:'#EEF2FF',borderRadius:10,padding:'10px 12px',marginBottom:6}}>
                      ...
                      {cSessions.filter(s=>s.done).length===0&&<div style={{color:'#9CA3AF',textAlign:'center',padding:'20px 0'}}>Історії ще немає</div>}
                    </div>
                  )}
                </div>
              )}
      ...
      {showAdd && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:20}}>
          <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:20,width:'100%',maxWidth:480,padding:24}}>
                  ...
                  {pricePlans.map(p=>(
                    <div key={p.id} onClick={()=>setNc({...nc,planId:p.id,clip:p.sessions})}
                      style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 12px',borderRadius:10,cursor:'pointer',
                        border:`1.5px solid ${nc.planId===p.id?'#C4ED00':'#C7D2FE'}`,
                        background:nc.planId===p.id?'rgba(196,237,0,.15)':'#EEF2FF'}}>
                      <div>
                        <div style={{fontSize:13,fontWeight:600,color:'#111827'}}>{p.name}</div>
            ...
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>setShowAdd(false)} style={{flex:1,padding:'10px',borderRadius:10,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',fontFamily:'DM Sans',fontSize:13,fontWeight:600,cursor:'pointer'}}>Скасувати</button>
      ...
      {showAddRecord && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:20}}>
          <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:20,width:'100%',maxWidth:440,padding:24}}>
            ...
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>setShowAddRecord(null)} style={{flex:1,padding:'10px',borderRadius:10,border:'1px solid #C7D2FE',background:'#EEF2FF',color:'#111827',fontFamily:'DM Sans',fontSize:13,fontWeight:600,cursor:'pointer'}}>Скасувати</button>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:20}}>
              {UNITS.map(u=>(
                <button key={u} onClick={()=>setNm({...nm,unit:u})}
                  style={{padding:'7px 14px',borderRadius:10,border:`1.5px solid ${nm.unit===u?'#C4ED00':'#C7D2FE'}`,background:nm.unit===u?'rgba(200,255,71,.12)':'#EEF2FF',color:nm.unit===u?'#C4ED00':'#9CA3AF',fontSize:13,fontWeight:nm.unit===u?700:400,cursor:'pointer'}}>
                  {u}
                </button>
              ))}
            </div>
      ...
      {openMetricChart && (() => {
        ...
        return (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:350,padding:16}} onClick={()=>setOpenMetricChart(null)}>
            <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:20,width:'100%',maxWidth:420,padding:24}} onClick={e=>e.stopPropagation()}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
                <div>
                  <div style={{fontWeight:700,fontSize:18,color:'#111827'}}>{metric.name}</div>
                ...
                {delta!==null && (
                  <div style={{textAlign:'right'}}>
                    ...
                    <div style={{color:'#9CA3AF',fontSize:11}}>з початку</div>
                  </div>
                )}
              </div>
              ...
              {sorted.length >= 2 ? (
                <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{overflow:'visible',marginBottom:12}}>
                  {[0,0.5,1].map((t,i)=>{
                    ...
                    return <g key={i}>
                      <line x1={PL} y1={y} x2={W-PR} y2={y} stroke="#C7D2FE" strokeWidth="1" strokeDasharray="4,4"/>
                  ...
                  {pts.map((p,i)=>(
                    <circle key={i} cx={p.x} cy={p.y} r="3.5" fill={lc} stroke="#FFFFFF" strokeWidth="2"/>
                  ))}
                  {pts.map((p,i)=>(
                    (i===0||i===pts.length-1) &&
                ...
                <div style={{color:'#9CA3AF',textAlign:'center',padding:'20px 0',fontSize:13}}>Потрібно мінімум 2 виміри для графіку</div>
              )}
              ...
              <div style={{borderTop:'1px solid #E0E7FF',paddingTop:12,marginBottom:12}}>
                ...
                <div style={{maxHeight:160,overflowY:'auto',display:'flex',flexDirection:'column',gap:6}}>
                  {[...sorted].reverse().map(m=>(
                    <div key={m.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#EEF2FF',borderRadius:8,padding:'8px 12px'}}>
                      <div style={{color:'#9CA3AF',fontSize:12}}>{m.date.slice(5).replace('-','/')}.{m.date.slice(0,4)}</div>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        <span style={{color:'#111827',fontWeight:600,fontSize:15}}>{m.value} <span style={{color:'#9CA3AF',fontSize:11,fontWeight:400}}>{metric.unit}</span></span>
              ...
              <button onClick={()=>setOpenMetricChart(null)}
                style={{width:'100%',padding:10,borderRadius:12,border:'1px solid #C7D2FE',background:'transparent',color:'#9CA3AF',fontSize:13,cursor:'pointer'}}>
                Закрити
              </button>
            </div>
          </div>
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
        {[['Сьогодні',todayCount,'#C4ED00'],['Тиждень',weekCount,'#47d4ff'],['Місяць',monthCount,'#3de87a']].map(([l,v,cl])=>(
          <div key={l} style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16,textAlign:'center'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:16}}>
        {/* Activity chart */}
        <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16}}>
          ...
          <div style={{display:'flex',alignItems:'flex-end',gap:8,height:120,marginBottom:8}}>
            {counts.map((c,i)=>(
              <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end',height:'100%',gap:4}}>
                ...
                <div style={{width:'100%',borderRadius:'4px 4px 0 0',minHeight:4,height:`${Math.max(4,(c/max)*90)}px`,background:days[i].ds===today?'#C4ED00':'#E0E7FF',border:`1px solid ${days[i].ds===today?'#C4ED00':'#C7D2FE'}`}}/>
              </div>
            ))}
          </div>
          <div style={{display:'flex',gap:8}}>
        ...
        <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16}}>
          ...
          {finance.length===0&&<div style={{color:'#9CA3AF',textAlign:'center',padding:'20px 0'}}>Фінансів ще немає</div>}
        </div>
        ...
        <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16}}>
          ...
          {pricePlans.length===0 && <div style={{color:'#9CA3AF',textAlign:'center',padding:'20px 0',fontSize:13}}>Прайс-листів ще немає</div>}
          {pricePlans.map(p=>(
            <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:'#EEF2FF',borderRadius:10,marginBottom:6}}>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:'#111827'}}>{p.name}</div>
              ...
              <button onClick={()=>{setEditPlan(p);setNp({name:p.name,sessions:p.sessions,price:p.price});setShowAddPlan(true)}}
                style={{background:'none',border:'1px solid #C7D2FE',borderRadius:7,color:'#9CA3AF',fontSize:12,padding:'4px 8px',cursor:'pointer'}}>✏️</button>
              <button onClick={()=>deletePlan(p.id)}
                style={{background:'none',border:'none',color:'#9CA3AF',fontSize:14,cursor:'pointer'}}>✕</button>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:20}}>
        <div style={{fontFamily:'Bebas Neue',fontSize:28,letterSpacing:1,color:'#C4ED00'}}>COACH<span style={{color:'#111827'}}>PRO</span></div>
        <div style={{fontSize:12,color:'#9CA3AF',marginTop:2}}>{DAYS_FULL[now.getDay()]}, {now.getDate()} {MONTHS_UK2[now.getMonth()]}</div>
      </div>

      {/* Section tabs */}
      <div style={{display:'flex',gap:4,background:'#EEF2FF',borderRadius:12,padding:4,marginBottom:20}}>
            ...
            <div style={{display:'flex',alignItems:'flex-end',gap:8,height:100,marginBottom:8}}>
              {counts.map((c,i)=>(
                <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end',height:'100%',gap:3}}>
                  ...
                  <div style={{width:'100%',borderRadius:'4px 4px 0 0',minHeight:4,height:`${Math.max(4,(c/maxC)*85)}px`,background:days[i].ds===today?'#C4ED00':'#E0E7FF',border:`1px solid ${days[i].ds===today?'#C4ED00':'#C7D2FE'}`}}/>
                </div>
              ))}
            </div>
            <div style={{display:'flex',gap:8}}>
        ...
        <div>
          ...
          {editFinance && (
            <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300}} onClick={()=>setEditFinance(null)}>
              <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:16,padding:24,width:300}} onClick={e=>e.stopPropagation()}>
                <div style={{fontFamily:'Bebas Neue',fontSize:20,marginBottom:16,color:'#111827'}}>Редагувати транзакцію</div>
                <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16}}>
                  <input value={ef.name} onChange={e=>setEf(p=>({...p,name:e.target.value}))} placeholder="Назва"
                    style={{padding:'9px 12px',borderRadius:10,border:'1px solid #C7D2FE',background:'#F8FAFF',color:'#111827',fontSize:13}}/>
        ...
        <div>
          <div style={{background:'#FFFFFF',border:'1px solid #C7D2FE',borderRadius:14,padding:16}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:7}}>
              {visiblePlans.map(p=>(
                <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'11px 12px',background:'#EEF2FF',border:'1px solid #C7D2FE',borderRadius:11}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:600,color:'#111827'}}>{p.name}</div>
                    <div style={{fontSize:11,color:'#9CA3AF',marginTop:2}}>{p.sessions} {p.sessions===1?'тренування':'тренувань'}</div>
                  </div>
            ...
            {pricePlans.length > 3 && (
              <button onClick={()=>setExpanded(!expanded)}
                style={{width:'100%',marginTop:10,padding:'9px',borderRadius:10,border:'1px solid #C7D2FE',background:'transparent',color:'#9CA3AF',fontSize:13,cursor:'pointer'}}>
...
export default function App() {
  ...
  return (
    <div style={{display:'flex',height:'100dvh',background:'#F0F4FF',color:'#111827',fontFamily:'DM Sans,sans-serif'}}>
      <div className="desktop-sidebar" style={{width:220,background:'#FFFFFF',borderRight:'1px solid #C7D2FE',display:'flex',flexDirection:'column',flexShrink:0,position:'sticky',top:0,height:'100dvh'}}>
        ...
        <div style={{padding:'16px 20px',borderTop:'1px solid #E0E7FF'}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:'#9CA3AF'}}>CoachPro</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div className="mobile-tabs" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',background:'#FFFFFF',borderTop:'1px solid #E0E7FF',flexShrink:0}}>
          {TABS.map(([id,icon,label])=>(
