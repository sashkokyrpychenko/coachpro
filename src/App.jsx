    ...
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:6,marginBottom:16}}>
        {Array.from({length:7},(_,i)=>{
          ...
          return (
            <div key={i} onClick={()=>setSelDay(i)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,padding:'8px 2px',borderRadius:10,cursor:'pointer',border:`1px solid ${active?'#c8ff47':'#2a3045'}`,background:active?'#c8ff47':'#1e2330',color:active?'#111':'#eef0f7'}}>
      ...
      <div style={{background:'#181c24',border:'1px solid #2a3045',borderRadius:14,padding:16,marginBottom:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          <span style={{fontFamily:'Bebas Neue',fontSize:18}}>{selDate.getDate()} {MONTHS_UK[selDate.getMonth()]}</span>
        ...
        {daySessions.map(s=>{
          ...
          return (
            <div key={s.id} onClick={()=>toggleDone(s.id,s.done)} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderRadius:12,background:'#1e2330',border:`1px solid ${s.done?'#3de87a33':'#2a3045'}`,marginBottom:8,cursor:'pointer'}}>
...
function ClientsTab({ clients, setClients, sessions }) {
  ...
  return (
    <div>
      ...
      {filtered.length===0 && <div style={{color:'#5a6482',textAlign:'center',padding:'40px 0'}}>Клієнтів ще немає</div>}
                  ...
                  <div style={{background:'#1e2330',borderRadius:12,padding:14}}>
                    ...
                    <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:12}}>
                      {Array.from({length:c.clip_total},(_,i)=>(
                        <div key={i} style={{width:28,height:28,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,background:i<c.clip_used?'#c8ff47':i===c.clip_used?'rgba(71,212,255,.1)':'#252c3d',border:i<c.clip_used?'2px solid #c8ff47':i===c.clip_used?'2px solid #47d4ff':'2px solid #2a3045',color:i<c.clip_used?'#111':'#5a6482'}}>{i<c.clip_used?'✓':''}</div>
                      ))}
                    </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',backdropFilter:'blur(4px)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:200}}>
          <div style={{background:'#181c24',border:'1px solid #2a3045',borderRadius:'20px 20px 0 0',width:'100%',maxWidth:430,padding:20}}>
            ...
            <select value={nc.clip} onChange={e=>setNc({...nc,clip:e.target.value})} style={{width:'100%',background:'#1e2330',border:'1px solid #2a3045',borderRadius:10,padding:'10px 14px',color:'#eef0f7',fontFamily:'DM Sans',fontSize:14,outline:'none',marginBottom:16}}>
              <option value={8}>8 занять</option>
              <option value={10}>10 занять</option>
              <option value={12}>12 занять</option>
              <option value={20}>20 занять</option>
            </select>
            <div style={{display:'flex',gap:10}}>
