...
// ─── Programs Tab ─────────────────────────────────────────────────────────────
function ProgramsTab({ clientId, programs, setPrograms }) {
    ...
    <div>
      ...
      {showAdd && (
        <div style={{background:'#0D0D16',border:'1px solid #1A2E4A',borderRadius:12,padding:12,marginBottom:12,display:'flex',gap:8}}>
          <input value={newName} onChange={e=>setNewName(e.target.value)}
            ...
            placeholder="Назва програми (напр. Ноги, Спина...)" autoFocus
      ...
      {clientPrograms.map(prog => (
        <div key={prog.id} style={{background:'#0D0D16',border:'1px solid #1A2E4A',borderRadius:12,overflow:'hidden',marginBottom:10}}>
          {/* Header */}
          <div onClick={()=>setOpenId(openId===prog.id?null:prog.id)}
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <button onClick={e=>{e.stopPropagation();deleteProgram(prog.id)}}
                style={{background:'rgba(255,68,102,.1)',border:'1px solid rgba(255,68,102,.2)',borderRadius:8,padding:'4px 10px',color:'#FF4466',fontSize:11,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans'}}>
                Видалити
              </button>
          ...
          {openId === prog.id && (
            <div style={{padding:'0 10px 14px'}}>
              ...
              <button onClick={()=>{
                const exs=[...(prog.exercises||[]),{name:'',sets:3,reps:10,weight:0}]
                updateExercises(prog.id,exs)
              }} style={{marginTop:10,width:'100%',padding:'8px',borderRadius:8,border:'1px dashed #1A2E4A',background:'none',color:'#4A90B8',fontFamily:'DM Sans',fontSize:12,cursor:'pointer'}}>
                + Додати вправу
              </button>
            </div>
          )}
...
function SkeletonBox({ w='100%', h=16, r=8, mb=0 }) {
  return (
    <div style={{
      ...
      background:'linear-gradient(90deg,#111118 25%,#1A2030 50%,#111118 75%)',
  ...
  return (
    <div>
      ...
      {tab === 'schedule' && (
        <>
          {/* Тижневий календар */}
          <div style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16,marginBottom:16}}>
          ...
          <div style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16,marginBottom:16}}>
            {Array.from({length:3}).map((_,i)=>(
              <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderBottom:i<2?'1px solid #1A2E4A':'none'}}>
          ...
          <div style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16}}>
            ...
            {Array.from({length:6}).map((_,i)=>(
              <div key={i} style={{display:'flex',gap:12,marginBottom:16}}>
                ...
                <div style={{flex:1,height:1,background:'#1A2E4A',marginTop:4}}/>
              </div>
            ))}
          </div>
        </>
      ...
      {tab === 'clients' && (
        <>
          ...
          {Array.from({length:4}).map((_,i)=>(
            <div key={i} style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16,marginBottom:12,display:'flex',alignItems:'center',gap:14}}>
      ...
      {tab === 'profile' && (
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
            {Array.from({length:3}).map((_,i)=>(
              <div key={i} style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16,display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div style={{background:'#111118',border:'1px solid #1A2E4A',borderRadius:14,padding:16}}>
            ...
            {Array.from({length:3}).map((_,i)=>(
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:i<2?'1px solid #1A2E4A':'none'}}>
